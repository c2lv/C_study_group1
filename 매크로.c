#include <windows.h> // window 환경에서만 사용 가능
#include <stdio.h>
...
int main(void)
{
   ...
   while (1)                             //무한반복 break문이나 return문으로 나가야됨.
   {
      ...
      if (GetKeyState(VK_F3) < 0) // 마우스 녹화
      {
         ...
         while (1) //무한반복.
         {
            ...
            if (GetKeyState(VK_LBUTTON) < 0 && check_mouse >= 1 || GetKeyState(VK_RBUTTON) < 0 && check_mouse2 >= 1) // 왼쪽버튼,오른쪽 버튼  눌렀을 때
            {
               ...
               else if (GetKeyState(VK_RBUTTON) < 0 && check_mouse2 >= 1)
               { //오른쪽만
                  sub_x2[i] = x[i - 1];
                  sub_y2[i] = y[i - 1];
                  x[i] = 7000;
      ...
      else if (GetKeyState(VK_F8) < 0) // 매크로 재생
      {
         ...
         while (1)
         {
            if (x[i] == 7000) // 오른쪽버튼 눌러지는 것을 7000이라고 가정
            {
               mouse_event(MOUSEEVENTF_RIGHTDOWN, sub_x2[i], sub_y2[i], 0, 0); // 좌표에 맞게 왼쪽버튼 누르기
               Sleep(1);
            }
            else if (x[i] == 8000) // 오른쪽버튼 떼지는 것을 8000이라고 가정
            {
...
// 자동으로 줄바꿈 처리를 해주는 함수(puts)
void println(char* s)
{
   printf("%s\n", s);
}

// ESC Key 입력 시 프로그램 종료
void exit_program()
{
...
// 원본 코드 출처
// https://intunknown.tistory.com/334#google_vignette
